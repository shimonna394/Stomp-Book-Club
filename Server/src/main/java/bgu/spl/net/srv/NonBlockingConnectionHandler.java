...
public class NonBlockingConnectionHandler<T> implements ConnectionHandler<T> {
    ...
    public NonBlockingConnectionHandler(MessageEncoderDecoder<T> reader, StompMessagingProtocolImpl protocol, SocketChannel chan, Reactor reactor) {
        this.chan = chan;
        this.encdec = reader;
            ...
            return () -> {
                try {
                    while (buf.hasRemaining()) {
                        T nextMessage = encdec.decode(buf.get());
                        if (nextMessage != null) {
